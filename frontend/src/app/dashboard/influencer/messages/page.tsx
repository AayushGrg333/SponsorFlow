...
export default function MessagesPage() {
     ...
     useEffect(() => {
          ...
          // 🔥 GUARANTEE chat view if URL intent exists
          if (newConvUserId || conversationId) {
          ...
          try {
               ...
               if (data && !response.error) {
                    ...
                    if (
                         data.length > 0 &&
                         !selectedConversation &&
                         !searchParams.get("newConversation") &&
     ...
     const loadUserDetailsAndCreateTempConversation = async (
          ...
          try {
               ...
               if (userType === "influencer") {
                    ...
                    if (response.data && !response.error) {
                         const data = response.data as any;
                         userName = data.companyName || "Unknown";
                         userAvatar = data.profileImage || "/placeholder.svg";
                    }
               }

               // Create temporary conversation object with fetched details
     ...
     const handleNewMessage = (newMsg: any) => {
          ...
          // ✅ IMPORTANT PART: upgrade "new" → real conversation
          if (selectedConversation?._id === "new" && newMsg.conversationId) {
               // Reload conversations and select the real one
               messagesAPI.getConversations().then((res) => {
                    const convs = (res.data as any)?.data ?? [];
     ...
     const handleSend = () => {
          ...
          // Reload conversations after sending to get the new conversation ID
