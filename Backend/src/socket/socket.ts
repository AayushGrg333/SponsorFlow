...
export const setupSocket = (io: Server) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("register_user", async (data: UserSocket) => {
      ...
      console.log(`User registered: ${data.userId} (${data.usertype})`);
        ...
        try {
          ...
          if (!conversation) {
            conversation = await ConversationModel.create({
              participants: [
                { id: new mongoose.Types.ObjectId(senderId), model: senderType },
                { id: new mongoose.Types.ObjectId(receiverId), model: receiverType },
              ],
            });
          }
          ...
          for (const [socketId, user] of connectedUsers.entries()) {
            if (user.userId === senderId || user.userId === receiverId) {
              io.to(socketId).emit("receive_message", transformedMessage);
            }
          }
        } catch (error) {
